...
io.on("connect", (socket) => {
  ...
  socket.on("comment", async (data) => {
    ...
    const liveStreamingHistory = await LiveStreamingHistory.findById(dataOfComment?.liveStreamingId);
  ...
  socket.on("normalUserGift", async (data) => {
    ...
    const [coin, senderUser, receiverUser] = await Promise.all([Math.abs(data.coin), User.findById(data.senderUserId).populate("level"), User.findById(data.receiverUserId).populate("level")]);
    ...
    if (senderUser && receiverUser && coin <= senderUser?.diamond) {
      ...
      await Promise.all([senderUser.save(), receiverUser.save(), outgoing?.save(), income.save(), LiveUser.updateOne({ liveUserId: receiverUser._id }, { $inc: { rCoin: coin } })]);
  ...
  socket.on("lessView", async (data) => {
    ...
    socket1?.length ? socket1[0].join(data?.liveStreamingId) : console.log("socket1 not able to join in lessView");
    ...
    const [liveUser, dataOfLiveUser] = await Promise.all([
      LiveUser.findOne({
    ...
    await io.in(data?.liveStreamingId).emit("view", liveUser ? liveUser.view : []);
  ...
  socket.on("getUserProfile", async (data) => {
    ...
    const user = await User.findById(data?.toUserId).populate("level").select("name username uniqueId gender age image country bio followers following video post level isVIP");
    if (user) {
      ...
      io.in("globalRoom:" + data?.fromUserId.toString()).emit("getUserProfile", userData);
    }
  });
  ...
  socket.on("blockedList", async (data) => {
    ...
    sockets?.length ? sockets[0].join(data?.userId) : console.log("sockets not able to emit");
  ...
  socket.on("callReceive", async (data) => {
    ...
    if (callDetail) {
      ...
      if (user && user.diamond >= data.coin) {
        ...
        await User.updateOne({ _id: callDetail.otherUserId }, { $inc: { diamond: data.coin } });
  ...
  socket.on("liveHostEnd", async (data) => {
    ...
    if (liveStreamingHistory) {
      ...
      liveStreamingHistory.duration = moment.utc(moment(new Date(liveStreamingHistory.endTime)).diff(moment(new Date(liveStreamingHistory.startTime)))).format("HH:mm:ss");
      await liveStreamingHistory.save();
  ...
  socket.on("disconnect", async () => {
    ...
    if (globalRoom) {
      ...
      if (socket1?.length == 0) {
        console.log("socket1?.length in Final disconnect:    ", socket1?.length);
        ...
        if (liveUser) {
          ...
          //@todo : liveEndTime
          const liveStreamingHistory = await LiveStreamingHistory.findById(liveUser?.liveStreamingId);
          if (liveStreamingHistory) {
            console.log("liveStreamingHistory in liveHostEnd:   ");
