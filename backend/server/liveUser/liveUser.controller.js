...
exports.generateToken = async (req, res) => {
  try {
    ...
    const token = RtcTokenBuilder.buildTokenWithAccount(global.settingJSON.agoraKey, global.settingJSON.agoraCertificate, req.body.channelName, account, role, privilegeExpiredTs);
...
exports.userIsLive = async (req, res) => {
  try {
    if (req.body.userId && req.body.channel) {
      ...
      const liveUser_ = await LiveUser.aggregate([matchQuery, { $addFields: { view: { $size: "$view" } } }]);

      return res.status(200).json({ status: true, message: "Success!!", liveUser: liveUser_[0] });
    } else {
      return res.status(200).json({ status: false, message: "Invalid Details!" });
    }
  } catch (error) {
...
exports.getLiveUser = async (req, res) => {
  try {
    const [user, followers, fakeLiveUser] = await Promise.all([
            ...
            pipeline: [
              {
                $match: {
                  $expr: {
                    $and: [{ $eq: ["$$userId", "$toUserId"] }, { $eq: [user._id, "$fromUserId"] }],
                  },
                },
              },
            ],
