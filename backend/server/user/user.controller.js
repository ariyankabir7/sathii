...
exports.index = async (req, res) => {
  try {
    ...
    if (!user) return res.status(200).json({ status: false, message: "Data not found!" });
...
exports.getPopularUser = async (req, res) => {
  try {
    if (!req.query.userId) {
      return res.send(200).json({ status: false, message: "userId is required" });
    }
...
exports.loginSignup = async (req, res) => {
  try {
    ...
    if (!req.body.identity) return res.status(200).json({ status: false, message: "Invalid Details!", user: {} });
    if (!req.body.email && !req.body.mobileNumber) return res.status(200).json({ status: false, message: "Invalid Details!", user: {} });

    let userExist, userNameExist, flag;
    ...
    if (userExist) {
      ...
      userExist.lastLogin = new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" });
      await userExist.save();

      return res.status(200).json({ status: true, message: "Success", user: userExist });
    }

    flag = await Flag.findOne({ name: req.body?.country?.trim() });

    const newUser = new User();
    const randomChars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";

    if (req.body.username) {
      userNameExist = await User.exists({
      ...
      if (userNameExist) {
        let newUsername = "";
        for (let i = 0; i < 8; i++) {
          newUsername += randomChars.charAt(Math.floor(Math.random() * randomChars.length));
        }
    ...
    newUser.mobileNumber = req.body.mobileNumber ? JSON.parse(req.body.mobileNumber) : 0;

    newUser.analyticDate = new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" });
    newUser.diamond += global.settingJSON ? global.settingJSON.loginBonus : 0;

    newUser.coverImage = req.body.coverImage ? req.body.coverImage : `${config.baseURL}storage/coverImage.jpg`;
    var digits = Math.floor(Math.random() * 90000000) + 10000000;
    newUser.uniqueId = digits;

    const user = await userFunction(newUser, req.body);
    
    if (global.settingJSON && global.settingJSON.loginBonus > 0) {
      const income = new Wallet();
      income.userId = user._id;
      income.diamond = global.settingJSON ? global.settingJSON.loginBonus : 0;
      income.type = 5;
...
const userFunction = async (user, data) => {
  ...
  console.log(".........................userFunction ........", data.image, data.gender);
  user.image = data.image === "" || "undefined" ? (data?.gender?.toLowerCase() === "female" ? `${config.baseURL}storage/female.png` : `${config.baseURL}storage/male.png`) : data.image;
...
exports.checkUsername = async (req, res) => {
  try {
    if (!req.query.username) return res.status(200).json({ status: false, message: "Invalid Details!" });
    ...
    User.findOne({
      ...
      if (error) return res.status(200).json({ status: false, message: "Internal Server Error" });
      else {
        if (user) {
...
exports.getProfileUser = async (req, res) => {
  try {
    ...
    const [user, profileUser] = await Promise.all([
      ...
      User.findOne({ ...query })
        .populate("level")
        .select("name username gender age image country bio followers following video post level isVIP coverImage countryFlagImage"),
    ]);
...
exports.referralCode = async (req, res) => {
  try {
    ...
    if (!user.isReferral) {
      ...
      user.diamond += global.settingJSON ? global.settingJSON.referralBonus : 0;

      referralCodeUser.rCoin += global.settingJSON ? global.settingJSON.referralBonus : 0;
      referralCodeUser.referralCount += 1;

      let walletData = [
        {
          userId: referralCodeUser._id,
          rCoin: global.settingJSON ? global.settingJSON.referralBonus : 0,
          type: 6,
        ...
        {
          userId: user._id,
          diamond: global.settingJSON ? global.settingJSON.referralBonus : 0,
          type: 6,
...
exports.randomMatch = async (req, res) => {
  try {
    ...
    const [user, UserData, users] = await Promise.all([
      User.findById(req.query.userId),
      User.find({
        isFake: true,
        ...(gender && { gender }),
        ...
        .populate("level")
        .select("name username gender age image country bio followers following video isFake post level isVIP loginType link"),
      User.find({
