...
exports.store = async (req, res) => {
  try {
    ...
    if (chatTopic) {
      return res.status(200).json({ status: true, message: "Success!!", chatTopic: chatTopic });
    }
...
exports.getChatList = async (req, res) => {
  try {
    ...
    const [user, list, fakeData] = await Promise.all([
      User.findById(req.query?.userId),
      ChatTopic.aggregate([
        {
          $match: {
            $or: [{ senderUser: new mongoose.Types.ObjectId(req.query.userId) }, { receiverUser: new mongoose.Types.ObjectId(req.query.userId) }],
          },
        },
        {
          $lookup: {
            ...
            pipeline: [
              {
                $match: {
                  $expr: {
                    $cond: {
                      if: {
                        $eq: ["$$senderUserIds", new mongoose.Types.ObjectId(req.query.userId)],
                      },
