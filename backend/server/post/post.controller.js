...
exports.uploadPost = async (req, res) => {
  try {
    ...
    return res.status(200).json({ status: true, message: "Success!!", postData });
  } catch (error) {
    deleteFile(req.file);
    console.log(error);
    return res.status(500).json({ status: false, error: error.message || "Server Error" });
...
exports.updateFakePost = async (req, res) => {
  try {
    ...
    posts.showPost = req.body.showPost ? parseInt(req.body.showPost) : posts.showPost;
    posts.allowComment = req.body.allowComment ? req.body.allowComment : posts.allowComment;
    posts.isFake = true;
          ...
          pipeline: [
            {
              $match: {
                $expr: {
                  $and: [{ $eq: ["$post", "$$postId"] }, { $eq: ["$user", user._id] }],
                },
              },
            },
          ],
