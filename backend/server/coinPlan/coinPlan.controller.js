...
exports.index = async (req, res) => {
  try {
    const coinPlan = await CoinPlan.find({ isDelete: false }).sort({ diamonds: 1 });
...
exports.createCustomer = async (req, res) => {
  try {
    ...
    const stripe = require("stripe")(global.settingJSON?.stripeSecretKey);
    ...
    const paymentIntent = await stripe.paymentIntents.create({
      amount: req.body.currency === "inr" ? plan?.rupee * 100 : plan?.dollar * 100,
    ...
    return res.status(200).json({
      ...
      customer: customer.id,
      publishableKey: global.settingJSON?.stripePublishableKey,
    });
  } catch (error) {
    console.log(error);
...
exports.payStripe = async (req, res) => {
  try {
    if (req.body.userId && req.body.planId) {
      const [user, plan] = await Promise.all([User.findById(req.body.userId).populate("level"), CoinPlan.findById(req.body.planId)]);
      ...
      // const stripe = require('stripe')(global.settingJSON ? global.settingJSON.stripeSecretKey : '');
...
exports.payGooglePlay = async (req, res) => {
  try {
    ...
    // const options = {
    //   email: global.settingJSON.googlePlayEmail,
    //   key: global.settingJSON.googlePlayKey,
    // };
