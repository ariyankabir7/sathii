...
exports.payStripe = async (req, res) => {
  try {
    if (req.body.userId && req.body.planId) {
      ...
      user.isVIP = true;
      user.diamond += global.settingJSON.vipDiamond ? global.settingJSON.vipDiamond : 100;
...
exports.payGooglePlay = async (req, res) => {
  try {
    ...
    const [user, plan] = await Promise.all([User.findById(req.body.userId).populate("level"), VIPPlan.findById(req.body.planId)]);
    ...
    // const options = {
    //   email: global.settingJSON ? global.settingJSON.googlePlayEmail : "",
    //   key: global.settingJSON ? global.settingJSON.googlePlayKey : "",
    // };
