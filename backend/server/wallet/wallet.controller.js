...
exports.getDiamondFromAd = async (req, res) => {
  try {
    ...
    if (
      ...
      user.ad.date.split(",")[0] ===
        new Date()
          ...
          .split(",")[0] &&
      user.ad.count >= global.settingJSON.maxAdPerDay
    ) {
      return res.status(200).json({ status: false, message: "You exceed your Ad limit." });
    }

    user.diamond += global.settingJSON ? global.settingJSON.freeDiamondForAd : 0;
    user.ad.count += 1;
    ...
    income.diamond = global.settingJSON ? global.settingJSON.freeDiamondForAd : 0;
    income.type = 4;
...
exports.convertRcoinToDiamond = async (req, res) => {
  try {
    ...
    user.diamond += parseInt(req.body.rCoin / global.settingJSON.rCoinForDiamond);
    await user.save();

    let data = [
      {
        userId: user._id,
        diamond: parseInt(req.body.rCoin / global.settingJSON.rCoinForDiamond),
        type: 1,
        isIncome: true,
...
exports.sendGiftFakeHost = async (req, res) => {
  try {
    ...
    const senderUser = await User.findById(req.body.senderUserId).populate("level");
    const receiverUser = await User.findById(req.body.senderUserId).populate("level");
    if (!senderUser) {
      return res.send({
        status: true,
